...
class Truck(CarBase):
    ...
    def check_validation_for_body_whl(self):
        ...
        or new_splitted_body_whl[0].isdigit != True or new_splitted_body_whl[1].isdigit != True or new_splitted_body_whl[2].isdigit != True):
            flag = 1
        else:
            flag = 0
        ...
        for i in range(len(row) // len(row_list)):
            if row[0] == 'car':
                try:
                    ...
                    print("Paseengers seat count is not INT!")
                except ValueError as err:
                    print("Carrying parameter is not FLOAT!")
